    ...
    <div class="modal__wrapper">
        <div class="modal_content">
            ...
            <div class="delete_warning">
                <h5>Вы уверены что хотите удалить {{deleteRecipeName}}</h5>
            </div>
            <div class="delete_footer">
                <button class="btn btn-dark" @click="close">Отмена</button>
                <button class="btn btn-danger" @click="deleteRec">Удалить</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed:{
    ...mapGetters('recipe',['statusloading'])
    },
    methods: {
        close() {
            this.$emit('close')
        ...
        deleteRec() {
            ...
            this.deleteRecipe(this.deleteId)
        }
